...
class _AiAssistantPanelState extends State<_AiAssistantPanel> {
  ...
  static const List<_ChatShortcut> _shortcuts = [
    _ChatShortcut(
      label: 'STR/Welfare',
      prompt: 'Am I eligible for STR or eKasih?',
      icon: Icons.card_giftcard,
    ),
    _ChatShortcut(
      label: 'Student Loan',
      prompt: 'Tell me about PTPTN loan and waiving options.',
      icon: Icons.school,
    ),
    _ChatShortcut(
    ...
    _ChatShortcut(
      label: 'Tax Filing',
      prompt: 'I need to file my income tax.',
      icon: Icons.description,
    ),
    _ChatShortcut(
      label: 'Summons Payment',
      prompt: 'How do I check and pay my summons?',
      icon: Icons.receipt,
    ),
    _ChatShortcut(
      label: 'EPF/Retirement',
      prompt: 'Help me with my EPF account management.',
      icon: Icons.savings,
    ),
  ];
  ...
  Future<void> _loadProfile() async {
    try {
      ...
      final derivedIncome =
          _parseIncome(prefill['income']) ?? _parseIncome(profile['income']);
      final derivedAge =
          _parseAge(profile['age']) ?? _ageFromDob(profile['dob']);

      _userProfile = {...profile, 'income': derivedIncome, 'age': derivedAge};
    } catch (_) {
      _userProfile = const {};
    } finally {
      if (mounted) {
...
class _ChatBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      onTap: null, // keep original behavior (no tap logic defined)
      child: Align(
        alignment: alignment,
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              if (!message.isUser &&
                  (message.actionLabel?.isNotEmpty ?? false)) ...[
                const SizedBox(height: 10),
                Row(
                  children: [
                    Expanded(
